...
describe("Index", () => {
  ...
  describe("Exception 12/13", () => {
    ...
    test("(33) where fails the modulus 11 check, but passes the modulus 10 check", () => {
      const { isValid, validationStatus } = verifyBankAccount(
        "074456",
        "11104102",
      );
  ...
  describe("Exception 14", () => {
    test("(34) where the first check fails and the second check passes", () => {
      const { isValid, validationStatus } = verifyBankAccount(
        "180002",
        "00000190",
      );
